...
const NegocioLanding = () => {
  ...
  const loadConfiguracion = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/public/${slug}/configuracion`);
      setConfig(response.data);
    } catch (error) {
      if (error.response?.status === 404) setError("Negocio no encontrado.");
      else if (error.response?.status === 403) setError("Este negocio está temporalmente inactivo.");
      else setError("Error cargando el negocio.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#f4f4f7] flex flex-col font-sans antialiased">
      ...
      <header
        ...
        style={{
          ...
          backgroundPosition: config.banner_position || "center",
        }}
      >
        ...
        <div className="relative z-10 px-4">
          ...
          {config.slogan && (
            <p className="text-lg md:text-xl text-white/90 mb-10 font-medium max-w-2xl mx-auto">{config.slogan}</p>
          )}

          <button
            onClick={() => navigate(`/reservar?negocio=${slug}`)}
            className="relative px-14 py-5 rounded-full font-black text-white shadow-2xl transition-all text-xl active:scale-95 hover:scale-110"
            style={{
              backgroundColor: config.color_primario || "#e91e63",
              boxShadow: `0 15px 45px ${config.color_primario || "#e91e63"}80`,
            }}
          >
            Reservar un Turno
            <span
              className="absolute inset-0 rounded-full animate-ping opacity-25"
              style={{ backgroundColor: config.color_primario || "#e91e63" }}
            ></span>
          </button>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10 w-full">
          {config.telefono && (
            <ContactCard
              ...
              color="#25D366"
              link={`https://wa.me/${config.telefono?.replace(/\D/g, "")}`}
              icon={
          ...
          {config.direccion && (
            <ContactCard
              ...
              color={config.color_primario || "#e91e63"}
              icon={
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
    ...
    >
      ...
      >
        <svg
          viewBox="0 0 24 24"
          className="w-8 h-8"
          style={{ fill: isLocation ? "none" : color, stroke: isLocation ? color : "none" }}
        >
          {icon}
        </svg>
      </div>

...
const TurnoCard = ({ turno, onCancelar, mensajeTemplate }) => {
  ...
  const handleWhatsApp = () => {
    const phone = turno.cliente_whatsapp || turno.telefono;
    if (!phone) {
  ...
  return (
    <div className="card p-4 hover:shadow-lg transition-all border-l-4" style={{ borderLeftColor: profesionalColor }}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          ...
          {turno.dni && <p className="text-xs text-dark-500">DNI: {turno.dni}</p>}
        </div>
        <div className="text-right">
          <p className="font-bold text-dark-900">{turno.hora_inicio}</p>
          <p className="text-sm text-dark-600">{turno.duracion} min</p>
          {turno.precio && <p className="text-sm font-semibold text-brand-600">${turno.precio.toLocaleString()}</p>}
        </div>
      </div>

      <div className="flex items-center gap-2 text-xs mb-3">
        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: profesionalColor }} />
      ...
      <div className="flex gap-2">
        <button onClick={handleWhatsApp} className="btn-ghost btn-sm flex-1 text-green-600 hover:bg-green-50">
        ...
        {turno.estado === "pendiente" && (
          <button onClick={() => onCancelar(turno.id)} className="btn-ghost btn-sm text-red-600 hover:bg-red-50">
            Cancelar
          </button>
        )}
      </div>
